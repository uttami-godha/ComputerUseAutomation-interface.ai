...
- **Governance**: `version`, `status` (`draft`→`approved`), `confidence` (updated from replay stability), and `provenance` (model, run id, redaction-policy version). Unattended replay of a risky step requires `approved` or an explicit flag.
...
The four cases are demonstrated in `evidence/` (success, not-found business outcome, recovered interstitial, and a hard/gated step that escalates). Multi-run stability (`--repeat N`) reports a pass rate and updates the artifact's `confidence`.
...
Control is modeled explicitly (`src/escalation/`): a `ControlToken` names who holds the single live session (`automation` | `human`) — they never act simultaneously. On a stuck discovery step, a replay hard failure, or a gated risky/irreversible step, the system raises an `InterventionRequest` carrying the context to act on it (capability/goal, step, reason, current URL, a screenshot, and a visible-text excerpt), transfers the token to `human`, and **exposes the same live session** — the human drives the exact Playwright page the automation was using (directly in the headed browser, or via a minimal operator console that calls `surface.perform` on that page). On **resume**, the token returns to `automation`, the human's actions are recorded, and replay retries the step from the (possibly changed) live state. The operator console is intentionally minimal (the brief allows a mocked operator UI); the **control-transfer model and the handoff mechanism are real**. The `evidence/replay-escalation-scripted` run shows the full loop: gate → intervention → transfer → resume → success, with every transition in the event log.
...
- **Risk classification**: actions are `safe` / `risky` / `irreversible` from intent + the target control's text, decided **conservatively** — an irreversible match (delete/transfer/close) can never be downgraded, and unknown clicks default to `risky`. Replay gates risky steps on an `approved` artifact or an explicit `--allow-risky`, and irreversible steps on explicit confirmation; discovery refuses irreversible clicks unless the goal requires them. The demo flow deliberately stops at the sub-account **review/confirmation** screen, before the irreversible create.
...
- **Real browser evidence not pre-committed.** A browser can't launch in the author's sandbox, so committed `evidence/` is *engine-level* (real replay engine driving a scripted surface — proving the taxonomy, checkpoints, drift, and escalation). The genuine LLM discovery run and browser-driven replays are packaged as one command — `npm run gen-real-evidence` (see README) — which writes them into `evidence/`. **Next:** run and commit those.
...
- **Assisted fallback / codegen / approval workflow** are stubbed or partial: `confidence` and `draft→approved` exist and multi-run stability updates confidence, but there's no bounded single-step LLM recovery on replay failure and no page-object codegen. **Next:** a policy-checked, single-step assisted recovery recorded as evidence.
...
Testing is where it counts: `npm test` runs 34 assertions over the replay engine, error taxonomy, guardrails, redaction, tenant merge, canonicalization, validation, and the catalog — all offline.